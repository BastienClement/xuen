...
class MutationContext private (val parents: Set[Mutable[_]] = Set.empty) {
	...
	/**
	  ...
	  */
	protected def flush(): Unit = MutationContext.execute(mutated, { _ =>
...
object MutationContext {
	...
	private[signal] object Dummy extends MutationContext {
		override private[signal] def defer(signal: Deferred[_]): Unit = ()
		override private[signal] def register(obs: Set[Observer]): Unit = ()
